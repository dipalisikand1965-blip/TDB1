          ...
          {(path.step1.type === "confirm_condition" || path.step1.type === "select_one") && (
            <div>
              ...
              {(path.step1.options || []).map(opt => (
                <OptionRow key={opt} option={{ name:opt, icon:null, desc:"", mira:opt === pet?.healthCondition }} selected={selections.step1.includes(opt)} onSelect={handleSelect1} accentColor={path.accentColor} />
              ))}
      ...
      {currentStep === 3 && (
        <StepPanel stepNum={3} title={path.step3.title} desc={path.step3.desc} accentColor={path.accentColor}>
          {(path.step3.options || []).map(opt => (
            <OptionRow key={opt.name} option={opt} selected={selections.step3.includes(opt.name)} onSelect={handleSelect3} accentColor={path.accentColor} multi={path.step3.type === "multi_select"} />
          ))}
...
export default function GuidedNutritionPaths({ pet }) {
  ...
  const hiddenCount = allPaths.length - 3;
  ...
  return (
    <div style={{ marginBottom:32 }} data-testid="guided-nutrition-paths">
      ...
      <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:4 }}>
        <div style={{ fontSize:"clamp(1.125rem,2.5vw,1.375rem)", fontWeight:800, color:"#1A0A00", fontFamily:"Georgia,serif" }}>
          Guided Nutrition Paths
        </div>
        <button
          onClick={() => setShowAll(!showAll)}
          style={{ background:"none", border:"1.5px solid #E0D0C0", borderRadius:20, padding:"5px 14px", fontSize:12, fontWeight:600, color:"#888", cursor:"pointer" }}
        >
          {showAll ? "Show less" : `See all ${allPaths.length}`}
        </button>
      </div>
      <div style={{ fontSize:12, color:"#888", marginBottom:18 }}>
        Mira picked {allPaths.length} paths for {pet?.name || "your pet"}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {paths.map(path => (
          <div key={path.id}>
            {/* Path row */}
            <div
              onClick={() => setOpenPath(openPath === path.id ? null : path.id)}
              style={{
                background: openPath === path.id ? path.accentBg : "#fff",
                border:`1.5px solid ${openPath === path.id ? path.accentBorder : "#F0E8E0"}`,
                borderRadius: openPath === path.id ? "14px 14px 0 0" : 14,
                cursor:"pointer", overflow:"hidden", transition:"all 0.15s",
              }}
              data-testid={`nutrition-path-${path.id}`}
            >
              {openPath !== path.id && (
                <div style={{ height:64, background:path.photoBg, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
            ...
            {openPath === path.id && (
              <div style={{
                background:"#fff",
                border:`1.5px solid ${path.accentBorder}`,
                borderTop:"none",
                ...
                <button
                  onClick={() => { setOpenPath(null); setActivePath(path.id); }}
