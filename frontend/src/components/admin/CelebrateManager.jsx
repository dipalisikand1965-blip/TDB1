  ...
  const openProductModal = (product = null) => {
    if (product) {
      ...
      setProductForm({
        ...
        flavors: (product.flavors || product.options?.find(o => o.name === 'Flavour')?.values || []).join(', '),
        bases: (product.bases || product.options?.find(o => o.name === 'Base')?.values || []).join(', '),
        fresh_delivery_cities: (product.fresh_delivery_cities || []).join(', '),
  ...
  const saveProduct = async () => {
    try {
      ...
      const payload = {
        ...
        bases: productForm.bases ? productForm.bases.split(',').map(b => b.trim()).filter(Boolean) : [],
        fresh_delivery_cities: productForm.fresh_delivery_cities ? productForm.fresh_delivery_cities.split(',').map(c => c.trim()).filter(Boolean) : [],
        life_stage: productForm.life_stage || null,
        occasion: productForm.occasion || null,
        dietary: productForm.dietary || null,
