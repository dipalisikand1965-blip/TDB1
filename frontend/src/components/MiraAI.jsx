...
const MiraAI = () => {
  ...
  const hiddenPaths = ['/admin', '/agent', '/login', '/mira', '/pet-home', '/dashboard', '/my-pets', '/my-requests'];
  const pillarPaths = ['/care', '/celebrate', '/celebrate-soul', '/advisory', '/dine', '/stay', '/travel', '/emergency', '/enjoy', '/fit', '/learn', '/farewell', '/adopt', '/paperwork', '/shop', '/all', '/product', '/services', '/cakes', '/treats', '/play', '/go'];
  ...
  const speakText = useCallback(async (text) => {
    ...
    if (useElevenLabs) {
      ...
      if (!success) {
        console.log('[MiraAI Voice] ElevenLabs unavailable — staying silent');
      }
      return;
    }
    // ElevenLabs not configured — stay silent
    console.log('[MiraAI Voice] ElevenLabs not configured — skipping TTS');
  ...
  if (!isOpen) {
    ...
    // HIDE on mobile - pillar/mobile navigation handles entry there
    return (
      <div 
