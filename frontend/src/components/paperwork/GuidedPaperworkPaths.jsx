...
function buildPaths(pet) {
  ...
  return [
    ...
    },
    {
      ...
      miraNote:`Pet insurance is most affordable when started young. ${insured?`${name} is insured — but I want to make sure the cover matches ${breed}'s specific health risks.`:`${name} isn't insured yet. An average emergency vet bill in India runs ₹15,000–₹80,000. Let me find the right policy.`}`,
      step1:{
...
function PathFlowModal({ path, pet, onClose }) {
  ...
  const handleSubmit=()=>setSubmitted(true); // TODO: POST /api/concierge/paperwork-path
