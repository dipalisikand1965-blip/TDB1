...
// ─── EXPORT ──────────────────────────────────────────────────
// ── PathCard — matches Care/Learn 3-column card style exactly ──
function PathCard({ path, pet, onOpen }) {
  const petName = pet?.name || 'your dog';
  return (
    <div onClick={onOpen}
      style={{ background:'#fff', borderRadius:16, border:`2px solid rgba(220,38,38,0.14)`,
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12 }}>
        <div style={{ width:50, height:50, borderRadius:14, background:path.iconBg||'#FEF2F2', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26 }}>{path.icon}</div>
      ...
      <div style={{ display:'flex', gap:5, marginBottom:12 }}>
        {(path.stepLabels||[]).map((label,i) => (
          <div key={i} style={{ flex:1 }}>
            <div style={{ height:3, borderRadius:3, marginBottom:3, background:i===0?path.accentColor:'rgba(220,38,38,0.15)' }}/>
    ...
    <section style={{ marginBottom:36 }}>
      <div style={{ display:'flex', alignItems:'baseline', gap:10, marginBottom:6 }}>
        <h3 style={{ fontSize:'clamp(1.125rem,2.5vw,1.375rem)', fontWeight:800, color:G.darkText, margin:0, fontFamily:'Georgia,serif' }}>
          Guided Emergency Paths
        </h3>
        <span style={{ fontSize:11, background:`linear-gradient(135deg,${G.crimson},${G.mid})`, color:'#fff', borderRadius:20, padding:'2px 10px', fontWeight:700 }}>
          {allPaths.length} paths
        </span>
      </div>
      <p style={{ fontSize:13, color:'#888', marginBottom:20, lineHeight:1.5 }}>
        Step-by-step emergency preparedness arranged by Concierge® — each personalised for {petName}.
      </p>
      <div style={{ display:'grid', gap:14 }} className="gep-grid">
        <style>{`.gep-grid{grid-template-columns:1fr}@media(min-width:480px){.gep-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(min-width:960px){.gep-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}`}</style>
        {allPaths.map(path => (
