      ...
      {PLAY_STRIPS.map(s => {
        ...
        return (
          <button key={s.id} onClick={() => setActive(sel ? null : s.id)}
            style={{ display:"inline-flex", alignItems:"center", gap:6, flexShrink:0, padding:"8px 16px", borderRadius:9999, border:`1.5px solid ${sel?"#E76F51":"rgba(231,111,81,0.28)"}`, background:sel?"#E76F51":"#fff", color:sel?G.mid:G.mutedText, fontSize:12, fontWeight:sel?700:400, cursor:"pointer", transition:"all 0.15s" }}>
