 ...
 *
 * Fully data-driven: reads ALL celebrate services from Service Box API.
 * Service Box admin (/admin → Celebrate → Services) is the single source of truth.
...
const CelebrateServiceGrid = ({ pet }) => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/api/service-box/services?pillar=celebrate&limit=20&is_active=true`);
        if (res.ok) {
          const data = await res.json();
          // Sort by sort_order (admin-controlled), then alphabetically
        ...
        console.error('[CelebrateServiceGrid] fetch error:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const openIntake = (serviceType) => {
    book({ service: `${petName} — ${serviceType}`, channel: 'celebrate_service_grid', urgency: 'normal' });
  ...
  if (loading) {
    return (
      <section style={{ padding: '0 0 48px' }} data-testid="celebrate-service-grid">
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <h2 style={{ fontSize: 'clamp(1.3rem,5vw,2rem)', fontWeight: 800, color: '#1A0030', fontFamily: 'Georgia, serif' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }} className="service-grid-responsive">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} style={{ height: 320, borderRadius: 16, background: '#F3E8FF', animation: 'pulse 1.5s infinite' }} />
          ))}
        </div>
      </section>
    );
  ...
  const cols = services.length <= 4 ? services.length : 4;
  ...
  return (
    <section style={{ padding: '0 0 48px' }} data-testid="celebrate-service-grid">
      ...
      {/* Grid — fully driven by Service Box data */}
      <div
      ...
      >
        {services.map((svc) => (
          <CelebrateServiceCard
            ...
            subLabel={svc.sub_label || svc.sub_title || ''}
            title={svc.name}
