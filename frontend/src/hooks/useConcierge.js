...
// ── Allergy extractor ──────────────────────────────────────────────────────
function getAllergiesFromPet(pet) {
...
// ═══════════════════════════════════════════════════════════════════════════
function buildMasterBriefing(pet, parent, intent, details = {}) {
  ...
  if (details.photo_url)    lines.push(`Photo:     ${details.photo_url}`);
  if (details.notes)        lines.push(`Notes:     ${details.notes}`);
  if (details.venue_name)   lines.push(`Venue:     ${details.venue_name}`);
...
function buildMasterMetadata(pet, parent, details = {}, callerMetadata = {}) {
  const allergies = getAllergiesFromPet(pet);
  return {
    // Pet
    pet_name:      pet?.name,
    ...
    // Caller-provided metadata wins for any key conflicts
    ...callerMetadata,
  };
}
