...
export function MiraPicksSection({ pet, onOpenService }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/api/admin/pillar-products?pillar=go&limit=200${breedParam}${allergenParam}`, { headers: auth })
        .then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`${API_URL}/api/services?pillar=go&limit=4`, { headers: auth })
        .then(r => r.ok ? r.json() : null).catch(() => null),
    ]).then(([pData, sData]) => {
      // applyMiraFilter: allergens BLOCKED → breed FIRST → loves FIRST → size/stage
        ...
        <div style={{ display:"flex", gap:14, overflowX:"auto", paddingBottom:10, scrollbarWidth:"thin" }} className="go-picks-scroll">
          ...
          {picks.map((pick, i) => {
            ...
            const displayScore = pick._miraRank !== undefined
              ...
              : (pick.mira_score || 0);
            const scoreColor = displayScore >= 80 ? "#16A34A" : displayScore >= 70 ? G.teal : "#6B7280";
            return (
              <div key={pick.id || i}
          ...
          <div onClick={e => e.stopPropagation()} style={{ width:"min(420px,100%)", borderRadius:20, background:"#fff", boxShadow:"0 20px 60px rgba(0,0,0,0.3)", overflow:"hidden" }}>
            <div style={{ background:`linear-gradient(135deg,${G.deep},${G.deepMid})`, padding:"20px 24px" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                <span style={{ fontSize:10, fontWeight:700, background:G.teal, color:G.deep, borderRadius:20, padding:"3px 10px" }}>SERVICE</span>
