...
function BookingModal({ service, pet, user, onClose, onBooked }) {
  const [step,     setStep]     = useState(1); // 1=confirm 2=notes 3=done
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/service_desk/attach_or_create_ticket`, {
        ...
        body: JSON.stringify({
          ...
          initial_message: {
            sender: "parent",
            text: `I'd like to book: ${service.name}${notes ? `. Notes: ${notes}` : ""}`
          }
        })
      });
      if (res.ok) { setStep(3); onBooked?.(service.name); }
      else setStep(3); // graceful — still show confirmation
    } catch { setStep(3); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)",
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:"28px 24px",
                    maxWidth:440, width:"100%", boxShadow:"0 24px 64px rgba(0,0,0,0.20)" }}>

        {step === 3 ? (
          <div style={{ textAlign:"center", padding:"16px 0" }}>
            <div style={{ width:56, height:56, borderRadius:"50%", background:"#E8F5E9",
                          display:"flex", alignItems:"center", justifyContent:"center",
                          fontSize:28, margin:"0 auto 16px" }}>✓</div>
            <div style={{ fontSize:18, fontWeight:800, color:G.deep, marginBottom:8 }}>
              Sent to Concierge
            </div>
            <div style={{ fontSize:13, color:G.mutedText, lineHeight:1.6, marginBottom:20 }}>
              Your request for <strong>{service.name}</strong> has been received.
              Concierge® will reach out on WhatsApp shortly.
            </div>
            <button onClick={onClose}
              style={{ width:"100%", padding:"12px", borderRadius:10,
                       background:G.deep, color:"#fff", border:"none",
          ...
          <>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:G.light, textTransform:"uppercase",
                              letterSpacing:"0.07em", marginBottom:4 }}>
                  Book via Concierge®
                </div>
                <div style={{ fontSize:17, fontWeight:800, color:G.deep }}>{service.name}</div>
              </div>
              <button onClick={onClose}
            ...
            {/* Service details */}
            <div style={{ background:G.pale, borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                <span style={{ fontSize:13, color:G.mutedText }}>For</span>
                <span style={{ fontSize:13, fontWeight:700, color:G.deep }}>{petName}</span>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontSize:13, color:G.mutedText }}>Price</span>
                <span style={{ fontSize:14, fontWeight:800, color:G.deep }}>
                  ₹{parseFloat(service.base_price||service.price||0).toLocaleString("en-IN")}
                </span>
              </div>
            </div>
            ...
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:12, fontWeight:600, color:G.mutedText,
                              display:"block", marginBottom:6 }}>
                Any notes? (optional)
              </label>
              <textarea value={notes} onChange={e => setNotes(e.target.value)}
                placeholder={`Anything Concierge should know about ${petName}…`}
                rows={3}
                style={{ width:"100%", padding:"10px 14px", borderRadius:10, fontSize:13,
                         border:`1.5px solid ${G.border}`, outline:"none", resize:"none",
                         color:G.darkText, fontFamily:"inherit", boxSizing:"border-box" }}/>
            </div>

            <button onClick={handleBook} disabled={loading}
              style={{ width:"100%", padding:"14px", borderRadius:10,
                       background:`linear-gradient(135deg,${G.deep},${G.slate})`,
                       color:"#fff", border:"none", fontSize:15, fontWeight:700,
                       cursor:loading?"wait":"pointer", opacity:loading?0.7:1 }}>
              {loading ? "Sending…" : "✦ Send to Concierge"}
            </button>
            <div style={{ textAlign:"center", marginTop:10, fontSize:11, color:G.light }}>
              Concierge® will confirm via WhatsApp
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ background:"#fff", border:`1px solid ${G.border}`, borderRadius:14,
      ...
      <div style={{ height:110, background: img ? "transparent" : `linear-gradient(135deg,${groupColour}22,${groupColour}44)`,
        ...
        <div style={{ position:"absolute", bottom:8, right:8, background:"rgba(255,255,255,0.95)",
