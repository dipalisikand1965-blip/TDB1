...
const ProductListing = ({ category = 'all' }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* === CELEBRATE CATEGORY NAVIGATION BAR === */}
      {pillar === 'celebrate' && (
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex gap-2 overflow-x-auto scrollbar-hide flex-1 pr-4">
                {CELEBRATE_SUBCATEGORIES.map((subcat) => {
                  const isActive = (subcat.id === 'all' && location.pathname === '/celebrate') ||
                    (subcat.id !== 'all' && (
                      location.pathname === subcat.path ||
                      (subcat.id === 'cakes' && location.pathname === '/celebrate/birthday-cakes') ||
                      (subcat.id === 'pupcakes' && location.pathname.includes('pupcakes')) ||
                      category === subcat.id
                    ));
