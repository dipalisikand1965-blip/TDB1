                  ...
                  >
                    ...
                    {userPets.map((pet) => (
                      <option key={pet.id} value={pet.id}>
                        🐕 {pet.name} {pet.breed ? `(${pet.breed})` : ''} {pet.identity?.weight ? `- ${pet.identity.weight}` : ''}
                      </option>
                    ))}
                    <option value="manual">✏️ Enter manually / New pet</option>
                  </select>
                  {selectedPetId && selectedPetId !== 'manual' && (
                    <p className="text-xs text-green-600 mt-2 flex items-center gap-1">
                      <CheckCircle className="w-3 h-3" />
                      Pet details loaded! You can still edit below.
                    </p>
                  )}
                </div>
              )}
