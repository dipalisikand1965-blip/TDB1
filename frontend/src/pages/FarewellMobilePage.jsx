...
export default function FarewellMobilePage() {
  ...
  const handleAddToCart=useCallback(p=>{addToCart({id:p.id||p._id,name:p.name,price:p.price||0,image:p.image_url||p.images?.[0],pillar:'farewell',quantity:1});},[addToCart]);
  ...
  return(
    <PillarPageLayout pillar="farewell" hideHero hideNavigation>
      <div className="farewell" data-testid="farewell-mobile"><style>{CSS_F}</style>
        ...
        {products.length>0&&(<div style={{padding:'0 16px 24px'}}><div style={{fontSize:18,fontWeight:700,marginBottom:12}}>Farewell Products for {petName}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>{products.slice(0,20).map(p=><SharedProductCard key={p.id||p._id||p.name} product={p} pillar="farewell" selectedPet={currentPet} onAddToCart={()=>handleAddToCart(p)} onClick={()=>{vibe();setSelectedProduct(p);}} />)}</div></div>)}

        {currentPet&&<div style={{padding:'0 16px 24px'}}><MiraImaginesBreed pet={currentPet} pillar="farewell" token={token}/></div>}
