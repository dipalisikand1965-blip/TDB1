...
function MiraPicksSection({ pet, onOpenService }) {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      // Emergency pillar boosts
      ranked.forEach(p => {
        const text = ((p.name||'') + ' ' + (p.description||'')).toLowerCase();
        if (text.includes('qr') || text.includes('id tag')) p._miraRank = Math.max(1, (p._miraRank||10) - 2);
        ...
        if ((pet?.health_data?.chronic_conditions||[]).length > 0) {
          if (text.includes('treatment') || text.includes('recovery') || text.includes('medical') || text.includes('helpline'))
  ...
  return (
    <section style={{marginBottom:28}}>
      ...
      {!picksLoading&&picks.length===0&&(<div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none",touchAction:"pan-y"}}>{imagines.map(item=><MiraImaginesCard key={item.id} item={item} pet={pet} token={token} pillar="emergency"/>)}</div>)}
