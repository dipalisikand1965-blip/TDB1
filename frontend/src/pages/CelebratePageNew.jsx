    ...
    <section style={{marginBottom:32}}>
      ...
      {!picksLoading&&picks.length>0&&(
        <div style={{display:"flex",gap:14,overflowX:"auto",paddingBottom:10,scrollbarWidth:"thin"}}>
          {picks.map((pick,i)=>{const isService=pick.entity_type==='service'||pick.type==='service';const displayScore=pick._miraRank!==undefined?Math.max(0,Math.round((15-pick._miraRank)/15*100)):(pick.mira_score||0);const col=displayScore>=80?"#16A34A":displayScore>=70?"#C44DFF":"#6B7280";const _ri=[pick.watercolor_image,pick.image_url,pick.image].find(u=>u&&u.startsWith("http")&&!u.includes("emergentagent.com")&&!u.includes("static.prod-images"))||null;const img=_ri&&(!_ri.includes("ai_generated") || _ri.includes("cloudinary.com"))?_ri:null;return(
            <div key={pick.id||i} style={{flexShrink:0,width:180,background:"#fff",borderRadius:16,border:"1.5px solid rgba(201,151,58,0.20)",overflow:"hidden",cursor:"pointer",boxShadow:"0 2px 12px rgba(201,151,58,0.08)",transition:"transform 0.15s,box-shadow 0.15s"}}
              ...
              <div style={{width:"100%",height:140,background:"#FFF8EE",overflow:"hidden",position:"relative"}}>{img?<img src={img} alt={pick.name||""} style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>e.target.style.display="none"}/>:<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#C9973A,#E8B96A)",color:"#fff",fontSize:12,fontWeight:700,padding:8,textAlign:"center"}}>{(pick.name||"").slice(0,18)}</div>}</div>
              <div style={{padding:"10px 11px 12px"}}><div style={{fontSize:12,fontWeight:700,color:"#1a0a2e",lineHeight:1.3,marginBottom:6,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{pick.name||"—"}</div>{!isService&&<div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}><div style={{flex:1,height:4,background:"#FFF8EE",borderRadius:4,overflow:"hidden"}}><div style={{width:`${displayScore}%`,height:"100%",background:col,borderRadius:4}}/></div><span style={{fontSize:10,fontWeight:800,color:col,minWidth:26}}>{displayScore}</span></div>}{pick._soulMatchReason&&<p style={{fontSize:10,color:"#D97706",margin:"0 0 4px",lineHeight:1.3,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>✦ {pick._soulMatchReason}</p>}{isService&&<p style={{fontSize:11,color:'#C9973A',lineHeight:1.45,margin:'0 0 8px'}}>Concierge® planning for {petName}'s celebration.</p>}<button onClick={(e)=>{e.stopPropagation();isService?onOpenService?.(pick.name):setSelPick(pick);}} style={{width:'100%',background:'linear-gradient(135deg,#C9973A,#E8B96A)',color:'#fff',border:'none',borderRadius:10,padding:'9px 10px',fontSize:12,fontWeight:700,cursor:'pointer',letterSpacing:'0.02em'}}>{isService?'Talk to Concierge® →':'View details →'}</button></div>
            </div>
          );})}
        </div>
      )}
