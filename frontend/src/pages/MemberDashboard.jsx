        ...
        >
          ...
          {pets.length > 1 && (
            <div className="mb-6 p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xl rounded-xl border border-white/10">
              ...
              <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-purple-500/50 -mx-1 px-1" style={{ WebkitOverflowScrolling: 'touch' }}>
                {pets.map((pet) => (
                  <button
                    key={pet.id}
                    onClick={() => setSelectedPetId(pet.id)}
                    className={`flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-full whitespace-nowrap transition-all flex-shrink-0 text-sm ${
                  ...
                  >
                    <span className="text-lg sm:text-xl flex-shrink-0" style={{ lineHeight: 1 }}>{pet.species === 'cat' ? '🐱' : '🐕'}</span>
