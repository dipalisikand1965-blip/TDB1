...
const FAREWELL_SERVICES = [
  ...
  { id:"cremation",     icon:"🌿", name:"Cremation Arrangement",          tagline:"Handled with care",                 price:"₹3,999", steps:2, accentColor:"#6366F1", desc:"Concierge® arranges the full cremation — collection, service, and return of remains — with complete dignity.", miraKnows:"Every detail will be handled with the love {name} deserves." },
...
function MiraPicksSection({ pet, onOpenService }) {
  ...
  return (
    <section style={{marginBottom:28}}>
      ...
      {!picksLoading&&picks.length===0&&<div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none",touchAction:"pan-y"}}>{imagines.map(item=><MiraImaginesCard key={item.id} item={item} pet={pet} token={token} pillar="farewell"/>)}</div>}
        ...
        {activeTab==="farewell" && (
          <>
            ...
            <div data-testid="farewell-soul-made-trigger" onClick={()=>setSoulMadeOpen(true)}
              ...
              <div style={{fontSize:10,fontWeight:800,letterSpacing:"0.15em",color:"#C44DFF",marginBottom:8}}>{`\u2726 SOUL MADE\u2122 \u00B7 IN MEMORY OF ${(petName||"YOUR DOG").toUpperCase()}`}</div>
