...
function MiraPicksSection({ pet, onOpenService }) {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      ranked.forEach(p => {
        ...
        if (text.includes('welcome') || text.includes('starter') || text.includes('new home') || text.includes('first')) p._miraRank = Math.max(2, (p._miraRank||10) - 2);
        const livesWithDogs = (pet?.doggy_soul_answers?.lives_with||[]).includes('Other dogs') || (pet?.doggy_soul_answers?.lives_with||'').includes('Other dogs');
        if (livesWithDogs && (text.includes('multi') || text.includes('pack') || text.includes('introduction') || text.includes('socializ')))
          p._miraRank = Math.max(2, (p._miraRank||10) - 1);
      });
  ...
  return (
    <section style={{marginBottom:28}}>
      ...
      {!picksLoading&&picks.length===0&&<div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none",touchAction:"pan-y"}}>{imagines.map(item=><MiraImaginesCard key={item.id} item={item} pet={pet} token={token} pillar="adopt"/>)}</div>}
      ...
      {!picksLoading&&picks.length>0&&(<div style={{display:"flex",gap:14,overflowX:"auto",paddingBottom:10,scrollbarWidth:"thin"}}>{picks.map((pick,i)=>{const isService=pick.entity_type==='service'||pick.type==='service';const displayScore=pick._miraRank!==undefined?Math.max(0,Math.round((15-pick._miraRank)/15*100)):(pick.mira_score||0);const col=displayScore>=80?"#16A34A":displayScore>=70?G.rose:"#6B7280";const _ri=[pick.cloudinary_url,pick.mockup_url,pick.image_url,pick.image].find(u=>u&&u.startsWith("http"))||null;const img=_ri&&(!_ri.includes("ai_generated") || _ri.includes("cloudinary.com"))?_ri:null;return<div key={i} style={{flexShrink:0,width:168,background:"#fff",borderRadius:14,border:`1.5px solid ${G.borderLight}`,overflow:"hidden",cursor:"pointer"}} onClick={()=>isService?onOpenService?.(pick.name):setSelPick(pick)} onMouseEnter={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseLeave={e=>e.currentTarget.style.transform=""}><div style={{width:"100%",height:130,background:G.pale,overflow:"hidden"}}>{img?<img src={img} alt={pick.name||""} style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>e.target.style.display="none"}/>:<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",background:`linear-gradient(135deg,${G.deep},${G.rose})`,color:"#fff",fontSize:12,fontWeight:700,padding:8,textAlign:"center"}}>{(pick.name||"").slice(0,24)}</div>}</div><div style={{padding:"10px 11px 12px"}}><div style={{fontSize:12,fontWeight:700,color:G.darkText,lineHeight:1.3,marginBottom:6,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{pick.name||"—"}</div>{isService?(<p style={{fontSize:11,color:G.mid,lineHeight:1.45,margin:'0 0 8px'}}>Compassionate concierge support for your adoption journey.</p>):(<div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}><div style={{flex:1,height:4,background:G.pale,borderRadius:4,overflow:"hidden"}}><div style={{width:`${displayScore}%`,height:"100%",background:col,borderRadius:4}}/></div><span style={{fontSize:10,fontWeight:800,color:col,minWidth:26}}>{displayScore}</span></div>)}{pick._soulMatchReason&&<p style={{fontSize:10,color:"#D97706",margin:"0 0 4px",lineHeight:1.3,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>✦ {pick._soulMatchReason}</p>}<button onClick={(e)=>{e.stopPropagation();if(isService){tdc.book({service:pick.name,pillar:'adopt',pet,channel:'adopt_mira_picks_service'});onOpenService?.(pick.name);}else{setSelPick(pick);}}} style={{width:'100%',background:`linear-gradient(135deg,${G.rose},${G.mid})`,color:'#fff',border:'none',borderRadius:10,padding:'8px 10px',fontSize:12,fontWeight:700,cursor:'pointer'}}>{isService?'Talk to Mira →':'View details →'}</button></div></div>;})})</div>)}
        ...
        {activeTab==="adopt" && (
          <>
            <div style={{background:"#fff",border:`1.5px solid ${G.borderLight}`,borderRadius:14,padding:"16px 20px",marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:8,overflowX:"auto",scrollbarWidth:"none",paddingBottom:4,touchAction:"pan-y"}}>
                {ADOPT_STAGES.map(s=>(
                  <button key={s.id}
              ...
              {openDim && (
                <div style={{marginTop:16,background:"#fff",border:`2px solid ${G.rose}`,borderRadius:20,padding:20}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:20}}>
                      {adoptSections.map(section=>(
                        <div key={section.id}>
                          ...
                          <div style={{display:"flex",gap:14,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none",touchAction:"pan-y"}}>
                  ...
                  {openDim==="home" && (
                    <>
                      <FirstTimePawrent pet={petData} token={token} accentColor="#D4537E" />
                      <div style={{marginTop:16,display:"flex",gap:14,overflowX:"auto",paddingBottom:8,scrollbarWidth:"none",touchAction:"pan-y"}}>
