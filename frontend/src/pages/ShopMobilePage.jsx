  ...
  return(
    <div>
      <div style={{background:`linear-gradient(135deg,${G.deep},${G.mid})`,borderRadius:16,padding:"20px",marginBottom:16,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            ...
            <div><div style={{fontSize:17,fontWeight:800,color:"#fff",fontFamily:"Georgia,serif"}}>The Doggy Bakery</div><div style={{fontSize:14,color:"rgba(255,255,255,0.65)"}}>thedoggybakery.com · Dog-safe · Handmade</div></div>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {["🐾 Dog-safe","✦ Handmade","🚚 Same-day BLR+MUM","🌱 No xylitol"].map(tag=><span key={tag} style={{background:"rgba(255,255,255,0.15)",borderRadius:20,padding:"3px 10px",fontSize:14,fontWeight:600,color:"#fff"}}>{tag}</span>)}
          </div>
        </div>
      </div>
      <div style={{background:G.pale,border:`1px solid ${G.border}`,borderRadius:10,padding:"10px 14px",marginBottom:12,display:"flex",gap:10,alignItems:"center"}}>
        ...
        <div style={{fontSize:14,color:G.darkText,lineHeight:1.5}}><strong>Streaties:</strong> 10% of every purchase feeds street animals. <a href="https://thedoggybakery.com/pages/streaties" target="_blank" rel="noopener noreferrer" style={{color:G.gold,fontWeight:600,textDecoration:"none"}}>Learn more →</a></div>
      </div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
        {BAKERY_FILTERS.map(f=><button key={f.id} onClick={()=>setFilter(f.id)} style={{padding:"5px 14px",borderRadius:20,fontSize:14,fontWeight:600,border:`1px solid ${filter===f.id?G.gold:G.border}`,background:filter===f.id?G.gold:G.pale,color:filter===f.id?"#fff":G.mid||G.darkText,cursor:"pointer"}}>{f.label}</button>)}
      </div>
      ...
      <div className="shop mobile-page-container" data-testid="shop-mobile">
        ...
        <div style={{background:`linear-gradient(160deg,${S.dark} 0%,${S.gold} 50%,${S.goldL} 100%)`,padding:'32px 16px 24px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
            <div><div style={{fontSize:14,fontWeight:700,color:'rgba(255,255,255,0.5)',letterSpacing:'0.1em',marginBottom:2}}>THE DOGGY COMPANY</div><div style={{fontSize:22,fontWeight:700,color:'#fff'}}>🛍️ Shop</div></div>
            {contextPets?.length>1&&(<select value={currentPet?.id} onChange={e=>{vibe();setCurrentPet(contextPets.find(p=>p.id===e.target.value));}} style={{background:'rgba(255,255,255,0.12)',border:'1px solid rgba(255,255,255,0.2)',borderRadius:999,padding:'7px 14px',color:'#fff',fontSize:14}}>{contextPets.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>)}
          </div>
        ...
        <div style={{margin:'0 16px 24px',background:S.dark,borderRadius:20,padding:18,cursor:'pointer'}} onClick={()=>setSoulMadeOpen(true)}>
          <div style={{fontSize:14,letterSpacing:'0.14em',color:S.goldXL,fontWeight:700,marginBottom:8}}>✦ SOUL MADE™ · MADE ONLY FOR {petName.toUpperCase()}</div>
