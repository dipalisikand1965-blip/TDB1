...
class TestSoulScoreAPI:
    ...
    def test_pets_with_varying_scores(self):
        ...
        if buddy:
            assert buddy.get("overall_score", 0) <= 10, f"Buddy should have low score, got {buddy.get('overall_score')}"
            print(f"✓ Buddy correctly has low score: {buddy.get('overall_score')}%")
    ...
    def test_score_tier_returned(self):
        ...
        for pet in pets:
            ...
            tier = pet["score_tier"]
            # Valid tiers: newcomer, explorer, soulmate, legend
            assert tier in ["newcomer", "explorer", "soulmate", "legend"], f"Invalid tier: {tier}"
            print(f"  - {pet.get('name')}: tier={tier}")
