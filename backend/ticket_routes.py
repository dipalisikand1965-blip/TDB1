...
# ============== CUSTOM STATUSES & CATEGORIES (Admin Configurable) ==============

@router.get("/statuses")
async def get_ticket_statuses():
    """Get all available ticket statuses including custom ones"""
    db = get_db()
    
    # Default statuses
    default_statuses = [
        {"id": "new", "label": "New", "color": "blue", "is_default": True},
        {"id": "open", "label": "Open", "color": "blue", "is_default": True},
        {"id": "in_progress", "label": "In Progress", "color": "yellow", "is_default": True},
        {"id": "pending", "label": "Pending", "color": "orange", "is_default": True},
        {"id": "waiting_on_member", "label": "Waiting on Member", "color": "purple", "is_default": True},
        {"id": "on_hold", "label": "On Hold", "color": "gray", "is_default": True},
        {"id": "resolved", "label": "Resolved", "color": "green", "is_default": True},
        {"id": "closed", "label": "Closed", "color": "gray", "is_default": True}
    ]
...
async def add_custom_status(data: dict):
    ...
    await db.service_desk_settings.update_one(
        {"type": "statuses"},
        {"$addToSet": {"statuses": {"id": status_id, "label": label, "color": color, "is_default": False}}},
        upsert=True
    )
...
async def delete_custom_status(status_id: str):
    ...
    default_ids = ["new", "open", "in_progress", "pending", "waiting_on_member", "on_hold", "resolved", "closed"]
...
async def get_ticket_categories():
    """Get all available ticket categories/pillars including custom ones"""
    db = get_db()
    ...
    custom_categories = settings.get("categories", []) if settings else []
    
    return {"categories": default_categories + custom_categories}
...
async def delete_custom_category(category_id: str):
    ...
    default_ids = ["celebrate", "dine", "stay", "travel", "care", "enjoy", "fit", "learn", "paperwork", "advisory", "emergency", "farewell", "adopt", "shop"]
