...
```bash
# Test Voice
curl -s "https://concierge-status-fix.preview.emergentagent.com/api/mira/voice/test"

# Test Remember
curl -s -X POST "https://concierge-status-fix.preview.emergentagent.com/api/mira/memory/remember" \
  -H "Content-Type: application/json" \
  -d '{"pet_id": "test", "memory_text": "Buddy is scared of thunder"}'

# Test Concierge Summarize
curl -s -X POST "https://concierge-status-fix.preview.emergentagent.com/api/mira/concierge/summarize" \
  -H "Content-Type: application/json" \
