...
- URL: `https://play-ui-sync.preview.emergentagent.com`
...
# Frontend  
REACT_APP_BACKEND_URL=https://play-ui-sync.preview.emergentagent.com
...
```bash
# Test Voice
curl -s "https://play-ui-sync.preview.emergentagent.com/api/mira/voice/test"

# Test Remember
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/mira/memory/remember" \
  -H "Content-Type: application/json" \
  -d '{"pet_id": "test", "memory_text": "Buddy is scared of thunder"}'

# Test Concierge Summarize
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/mira/concierge/summarize" \
  -H "Content-Type: application/json" \
...
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/mira/os/understand-with-products" \
  -H "Content-Type: application/json" \
...
TOKEN=$(curl -s -X POST 'https://play-ui-sync.preview.emergentagent.com/api/auth/login' \
  -H 'Content-Type: application/json' \
  -d '{"email":"dipali@clubconcierge.in","password":"test123"}' | \
  python3 -c 'import sys,json;print(json.load(sys.stdin).get("access_token",""))')

# 2. Test chat (should mention allergies without asking)
curl -X POST "https://play-ui-sync.preview.emergentagent.com/api/mira/chat" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"message": "What food for Lola?", "session_id": "test-123"}'

# 3. Test pets API (Lola should show ~62% score)
curl "https://play-ui-sync.preview.emergentagent.com/api/pets/my-pets" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/mira/chat" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
...
```
❌ DEPLOYED WITH: https://play-ui-sync.preview.emergentagent.com (DOESN'T EXIST!)
...
```
Failed to load resource: 404 at https://play-ui-sync.preview.emergentagent.com/api/auth/me
```
...
# 2. If it shows preview URL (like https://play-ui-sync.preview.emergentagent.com), FIX IT:
...
|-------------|----------------------|
| Preview | `https://play-ui-sync.preview.emergentagent.com` (varies) |
...
curl -X POST "https://thedoggycompany.in/api/admin/env-sync/full-sync?token=sync-preview-to-prod-2026&overwrite=true" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "source_url": "https://play-ui-sync.preview.emergentagent.com",
    "user_email": "dipali@clubconcierge.in",
...
curl -X POST "https://play-ui-sync.preview.emergentagent.com/api/admin/env-sync/full-sync?token=sync-preview-to-prod-2026&overwrite=true" \
  -u "aditya:lola4304" \
  -H "Content-Type: application/json" \
  -d '{
...
### To Test API:
```bash
API_URL="https://play-ui-sync.preview.emergentagent.com"
...
| Preview URL | https://play-ui-sync.preview.emergentagent.com/mira-demo |
...
- **Admin:** `aditya` / `lola4304`
- **Test URL:** `/mira-demo?debug=1`
- **API URL:** `https://play-ui-sync.preview.emergentagent.com`
...
- **Preview Environment:** https://play-ui-sync.preview.emergentagent.com
...
## Frontend
- **Base URL**: https://play-ui-sync.preview.emergentagent.com
- **Login**: /login
...
## Backend API
- **Base**: https://play-ui-sync.preview.emergentagent.com/api
...
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/membership/onboard" \
  -H "Content-Type: application/json" \
...
- **Main Demo**: https://play-ui-sync.preview.emergentagent.com/mira-demo
- **Pure OS Test**: https://play-ui-sync.preview.emergentagent.com/mira-pure-os
...
curl -s -u "aditya:lola4304" "https://play-ui-sync.preview.emergentagent.com/api/admin/notifications?limit=5"

# Test ticket enrichment
curl -s -X POST "https://play-ui-sync.preview.emergentagent.com/api/service_desk/resolve_ticket/TCK-XXX"

# Check pet soul data
curl -s "https://play-ui-sync.preview.emergentagent.com/api/pets/pet-3661ae55d2e2"
```
