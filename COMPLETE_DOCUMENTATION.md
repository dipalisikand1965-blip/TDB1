...
- URL: `https://dynamic-cms-platform.preview.emergentagent.com`
...
curl -s -X POST "https://dynamic-cms-platform.preview.emergentagent.com/api/mira/os/understand-with-products" \
  -H "Content-Type: application/json" \
...
TOKEN=$(curl -s -X POST 'https://dynamic-cms-platform.preview.emergentagent.com/api/auth/login' \
  -H 'Content-Type: application/json' \
  -d '{"email":"dipali@clubconcierge.in","password":"test123"}' | \
  python3 -c 'import sys,json;print(json.load(sys.stdin).get("access_token",""))')

# 2. Test chat (should mention allergies without asking)
curl -X POST "https://dynamic-cms-platform.preview.emergentagent.com/api/mira/chat" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"message": "What food for Lola?", "session_id": "test-123"}'

# 3. Test pets API (Lola should show ~62% score)
curl "https://dynamic-cms-platform.preview.emergentagent.com/api/pets/my-pets" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl -s -X POST "https://dynamic-cms-platform.preview.emergentagent.com/api/mira/chat" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
...
```
❌ DEPLOYED WITH: https://dynamic-cms-platform.preview.emergentagent.com (DOESN'T EXIST!)
...
```
Failed to load resource: 404 at https://dynamic-cms-platform.preview.emergentagent.com/api/auth/me
```
...
# 2. If it shows preview URL (like https://dynamic-cms-platform.preview.emergentagent.com), FIX IT:
...
|-------------|----------------------|
| Preview | `https://dynamic-cms-platform.preview.emergentagent.com` (varies) |
...
curl -X POST "https://thedoggycompany.in/api/admin/env-sync/full-sync?token=sync-preview-to-prod-2026&overwrite=true" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "source_url": "https://dynamic-cms-platform.preview.emergentagent.com",
    "user_email": "dipali@clubconcierge.in",
...
curl -X POST "https://dynamic-cms-platform.preview.emergentagent.com/api/admin/env-sync/full-sync?token=sync-preview-to-prod-2026&overwrite=true" \
  -u "aditya:lola4304" \
  -H "Content-Type: application/json" \
  -d '{
...
### To Test API:
```bash
API_URL="https://dynamic-cms-platform.preview.emergentagent.com"
...
| Preview URL | https://dynamic-cms-platform.preview.emergentagent.com/mira-demo |
...
- **Admin:** `aditya` / `lola4304`
- **Test URL:** `/mira-demo?debug=1`
- **API URL:** `https://dynamic-cms-platform.preview.emergentagent.com`
...
- **Preview Environment:** https://dynamic-cms-platform.preview.emergentagent.com
...
## Test URL
**https://dynamic-cms-platform.preview.emergentagent.com/mira-demo**
